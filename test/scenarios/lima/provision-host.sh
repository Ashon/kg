...
#
# Prepares a VM to be a kgenesis host: root SSH, and the packages kgenesis
...
# vmnet's DHCP does not answer on every macOS host, and the shared switch works
# regardless, so the address is assigned rather than requested.
